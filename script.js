...
function updateDashboard() {
    ...
    // --- MEMANGGIL FUNGSI UNTUK TABEL BARU ---
...
// --- FUNGSI BARU UNTUK TABEL AKSI ---
function updateActionableTables(data) {
    const outletSummary = {};

    // Menggabungkan data per outlet
    data.forEach(item => {
        if (!outletSummary[item.outletName]) {
            outletSummary[item.outletName] = {
                brand: item.brand,
                totalRevenue: 0,
                totalCogsValue: 0,
                totalLaborValue: 0,
        ...
        outletSummary[item.outletName].totalRevenue += item.revenue;
        outletSummary[item.outletName].totalCogsValue += item.revenue * item.cogs;
        outletSummary[item.outletName].totalLaborValue += item.revenue * item.laborCost;
        outletSummary[item.outletName].totalComplaints += item.complaints;
    });

    const summaryArray = Object.keys(outletSummary).map(outletName => {
        const outletData = outletSummary[outletName];
        const primeCost = outletData.totalRevenue > 0 ? ((outletData.totalCogsValue + outletData.totalLaborValue) / outletData.totalRevenue) * 100 : 0;
        return {
            outletName,
            brand: outletData.brand,
            primeCost: primeCost,
            totalComplaints: outletData.totalComplaints
        };
    });

    // Urutkan dan ambil 5 teratas untuk Prime Cost
    const topPrimeCost = summaryArray.sort((a, b) => b.primeCost - a.primeCost).slice(0, 5);
    populatePrimeCostTable('top-prime-cost-table', topPrimeCost);

    // Urutkan dan ambil 5 teratas untuk Komplain
    const topComplaints = summaryArray.sort((a, b) => b.totalComplaints - a.totalComplaints).slice(0, 5);
    populateComplaintsTable('top-complaints-table', topComplaints);
}

function populatePrimeCostTable(tableId, data) {
    const tableBody = document.getElementById(tableId);
    tableBody.innerHTML = '';
    data.forEach(item => {
        tableBody.innerHTML += `
            <tr class="text-sm">
                <td class="py-2 px-4 font-medium text-gray-800">${item.outletName}</td>
                <td class="py-2 px-4 text-gray-600">${item.brand}</td>
                <td class="py-2 px-4 font-semibold text-right text-red-600">${item.primeCost.toFixed(1)}%</td>
            </tr>`;
    });
}

function populateComplaintsTable(tableId, data) {
    const tableBody = document.getElementById(tableId);
    tableBody.innerHTML = '';
    data.forEach(item => {
        tableBody.innerHTML += `
            <tr class="text-sm">
                <td class="py-2 px-4 font-medium text-gray-800">${item.outletName}</td>
                <td class="py-2 px-4 text-gray-600">${item.brand}</td>
                <td class="py-2 px-4 font-semibold text-right text-red-600">${item.totalComplaints}</td>
            </tr>`;
    });
}
